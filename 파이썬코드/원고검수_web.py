...
def get_keywords_from_sheet():
    ...
    try:
        ...
        # 키워드와 사유/제안 가져오기
        keywords = [k.encode('utf-8').decode('utf-8') for k in sheet.col_values(2)[2:]]  # B3부터
        reasons = [r.encode('utf-8').decode('utf-8') for r in sheet.col_values(3)[2:]]   # C3부터
        
        # 딕셔너리로 변환
        keyword_notes = {}
        for keyword, reason in zip(keywords, reasons):
            if keyword and keyword.strip():  # 빈 셀 제외
                keyword = keyword.strip()
                reason = reason.strip() if reason else ''
                keyword_notes[keyword] = reason
...
def highlight_keywords(content, keyword_notes):
    ...
    try:
        ...
        try:
            ...
            style = doc.styles.add_style('Normal', 1)
        
        style.font.name = "맑은 고딕"
        style.font.ascii_font = "맑은 고딕"
        style.font.eastasia_font = "맑은 고딕"
            ...
            if positions:
                ...
                for start, end, keyword in positions:
                    ...
                    if keyword_notes[keyword]:
                        ...
                        note_run.font.ascii_font = "맑은 고딕"
                        note_run.font.eastasia_font = "맑은 고딕"
